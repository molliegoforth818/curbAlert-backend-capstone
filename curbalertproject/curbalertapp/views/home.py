    ...
    if request.method == 'GET':
        ...
        for donation in available_donation_queryset:
            available_donations.append({
                'id':donation.id,'description': donation.description, 'latitude': donation.alerter.latitude, 'longitude': donation.alerter.longitude, 'is_expired':donation.is_expired, 'user':donation.alerter.user.username, 'needs_haul_away': donation.needs_haul_away
            })        
        template = 'home.html'
        context = {
